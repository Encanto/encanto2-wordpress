...
<?php if (get_post_meta($post_id, "rnr_assign_type", true) == "parallax-section") { ?>
	...
	<div id="<?php echo $post_name; ?>" class="parallax">
		...
		<div class="container clearfix">
      	...
      	<?php
					if ( $post_name === 'successful-applications' )
					{
						echo "<h2>Successful Applications</h2>";
						$pathToSocialSphere = "social_sphere/";
						// Query arguments
						$args = array(
							'post_status' => 'publish',
							...
							'order' => 'ASC',
							'meta_key' => 'frontpage_post_order'
						);
						
						// Actual query result
						$succesfull_apps_query = new WP_Query($args); 
						
						if ( $succesfull_apps_query->have_posts() ) 
						{
							$index = 0;
							
							while ( $succesfull_apps_query->have_posts() )
							{
								if ($index % 2 === 0)
								{
									if ( $index > 0 ) 	echo '</ul>';
									echo '<ul class="posts ' .  (($index === 0 )?'first':'') . '">';
								}
								
								$succesfull_apps_query->the_post(); 
								//echo '<li data-url="' . get_permalink() . '">';
								echo '<li data-url="' . $pathToSocialSphere . '#post-' . get_the_id() . '">';
            ...
            <script>
						 jQuery(function($) {
								$('#successful-applications ul.posts li').on('click', function(e) {
